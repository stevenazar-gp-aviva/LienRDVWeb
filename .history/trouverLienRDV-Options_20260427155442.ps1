...
.NOTES
    ...
    Prérequis      : duckdb.exe (CLI), DuckDB.ps1, Module Sentry
...
$sentryConfig = Import-PowerShellDataFile -Path "D:\MyReport\Sources\Reporting\DIGITAL\Sources\configSentry.psd1"

Import-Module Sentry -Force

Start-Sentry {
    $_.Dsn              = $sentryConfig.Dsn
    $_.Environment      = $sentryConfig.Environment
    $_.Release          = $sentryConfig.Release
    $_.TracesSampleRate = $sentryConfig.TracesSampleRate
    $_.AttachStacktrace = $true
}
